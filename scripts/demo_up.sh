...
(cd polaris/backend && uv run uvicorn src.api.main:app --port 8000 >/tmp/polaris_api.log 2>&1 &)
sleep 6
curl -sf http://localhost:8000/health >/dev/null || { echo "API failed to start — see /tmp/polaris_api.log"; exit 1; }

echo "[2/4] opening Cloudflare tunnel..."
cloudflared tunnel --url http://localhost:8000 >/tmp/polaris_tunnel.log 2>&1 &
sleep 10
URL=$(grep -oE "https://[a-z0-9-]+\.trycloudflare\.com" /tmp/polaris_tunnel.log | head -1)
[ -n "$URL" ] && echo "    backend public URL: $URL" || { echo "tunnel failed — see /tmp/polaris_tunnel.log"; exit 1; }
...
npx -y vercel deploy --prod --yes --token "$VERCEL_TOKEN" 2>&1 | grep -oE "https://polaris-[a-z0-9-]+\.vercel\.app" | head -1 || true

echo
echo "demo ready:"
echo "  dashboard : https://polaris-omega-taupe.vercel.app"
echo "  api       : $URL"
